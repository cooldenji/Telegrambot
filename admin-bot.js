...
// QR Upload (Admin→User relay)
async function handleQrUpload(chatId,userId,method,adminFileId) {
  await send(chatId,`⏳ Uploading QR, please wait...`);
  ...
  if (userFileId) {
    ...
    db.saveDb();
    return send(chatId,`✅ QR uploaded! Both bots have valid copies.`,{reply_markup:{inline_keyboard:[[{text:'🔙 Back',callback_data:`pay_${method}`}]]}});
  } else {
...
async function showDepDetail(chatId,msgId,did) {
  ...
  const kb={inline_keyboard:[
    ...
    [{text:'✉️ Message User',url:`tg://user?id=${dep.userId}`}],
...
async function showOrdDetail(chatId,msgId,oid) {
  ...
  return edit(chatId,msgId,text,{reply_markup:{inline_keyboard:[
    [{text:'✅ Mark Delivered',callback_data:`deliver_${oid}`},{text:'✉️ Message User',url:`tg://user?id=${ord.userId}`}],
