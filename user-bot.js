...
// Without these, an uncaught error inside an async handler just prints
// a bare file/line reference (e.g. "user-bot.js:50") with no message —
// these log the FULL error so the real cause is visible in Railway logs.
...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
  ...
  // Referral "click" is counted immediately on first /start — this is
  // just a stat (how many people opened the link). The actual REWARD is
  // only credited once this same user has *verified-joined* both the
  // channel and group (see handleJoinCheck below) — that's what stops
  // someone farming referral rewards without ever joining anything.
  if (isNew && validReferredBy) db.incrementReferralClick(validReferredBy);

  // ── Mandatory join gate ───────────────────────────────────────────
  // Every /start (new AND returning users) is checked against Telegram
  // in real time. Nobody reaches the Main Menu until getChatMember
  // confirms they're actually in both the channel and the group.
  ...
  if (!status.allOk) {
    ...
    return bot.sendMessage(chatId,
      ...
      `🔒 To use this bot you must first join our ${missing.join(' and ')}.\n\n` +
      `1️⃣ Tap the buttons below to join\n2️⃣ Then tap *Check Status* to continue`,
      { parse_mode: 'Markdown', reply_markup: joinStepsKeyboard(s) }
    );
  }

  // ── Verified — unlock the account ─────────────────────────────────
...
// ── Join check — re-verifies against Telegram and unlocks the bot ────
async function handleJoinCheck(chatId, userId, messageId) {
  ...
  if (!status.allOk) {
    ...
    await navigate(chatId, messageId,
      `❌ You still haven't joined: ${missing.join(' and ')}.\n\n` +
      `Please join using the buttons below, then tap *Check Status* again.`,
      { parse_mode: 'Markdown', reply_markup: joinStepsKeyboard(s) }
    );
    return false;
  }

  // ── Verified just now — unlock the account (mirrors /start logic for
  // a user who joined AFTER their first /start, then tapped Check Status) ──
  ...
  if (user && !user.acceptedTerms) {
    ...
    if (user.referredBy && !user.referralCredited) {
      db.updateUser(userId, { referralCredited: true });
      db.creditReferral(user.referredBy, userId, s.referralRewardInr);
      notifyReferrer(user.referredBy, userId, s.referralRewardInr);
    }
  }

  await navigate(chatId, messageId,
    `✅ *Verified! You're all joined up.*`,
    { parse_mode: 'Markdown' }
  );
...
function sendMainMenu(chatId, userId) {
  ...
  let balanceText;
  try {
    balanceText = fmtPrice(user ? user.balanceInr : 0);
  } catch (e) {
    console.error('🔥 fmtPrice failed in sendMainMenu:', e && e.stack ? e.stack : e);
    balanceText = '₹0'; // safe fallback so the menu still sends
  }
  return bot.sendMessage(chatId,
    `🏠 *Main Menu*\n▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n` +
    `💰 Balance: *${balanceText}*\n\n` +
...
bot.on('callback_query', async (query) => {
  ...
  // Telegram callback queries expire (~a few seconds). If we do slow
  // work (file reads, multiple sendMessage calls) before answering,
  // answerCallbackQuery can throw "query is too old" — which our catch
  // block then misreports as "Something went wrong" even though the
  // actual menu/message was sent successfully. Answering immediately
  // avoids that false-positive error entirely.
  ...
  try {
    ...
    console.error('🔥 Callback error (data=' + data + '):', e && e.stack ? e.stack : e);
  }
});
