...
async function relayPhotoToAdmin(fileId, caption, options = {}) {
  ...
  return adminPushBot.sendPhoto(ADMIN_NOTIFY_CHAT_ID, link, { caption, ...options });
}
...
async function checkOneMembership(chatRef, userId) {
  ...
  try {
    ...
    return { ok: isActiveMember(member.status), skipped: false };
  } catch (err) {
    console.error(`getChatMember failed for ${chatRef}:`, err.message);
...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
  ...
  await bot.sendMessage(chatId,
    ...
    `1️⃣ Tap *Join Channel*\n2️⃣ Tap *Join Group*\n3️⃣ Come back and tap *Continue*`,
    { parse_mode: 'Markdown', reply_markup: joinStepsKeyboard(s) }
  );
});

// ── Join check + terms acceptance (combined into ONE step) ───────
async function handleJoinCheck(chatId, userId, messageId) {
  ...
  if (!status.allOk) {
    ...
    await navigate(chatId, messageId,
      `❌ *Not joined yet*\n\nYou still need to join: ${missing.join(' and ')}\n\n` +
      `Tap the buttons below, join, then tap *Continue* again.`,
      { parse_mode: 'Markdown', reply_markup: joinStepsKeyboard(s) }
    );
    return false;
  }
  ...
  // Credit the referrer's reward here — only once, guarded by
  // referralCredited so re-checking join status twice can't double-pay.
  if (freshUser && freshUser.referredBy && !freshUser.referralCredited) {
    db.updateUser(userId, { referralCredited: true });
    const s = db.getSettings();
    db.creditReferral(freshUser.referredBy, userId, s.referralRewardInr);
    notifyReferrer(freshUser.referredBy, userId, s.referralRewardInr);
  }
...
bot.on('message', async (msg) => {
  ...
  if (!user.acceptedTerms) return bot.sendMessage(chatId, '🔒 Please join our Channel & Group first — send /start to begin.');
